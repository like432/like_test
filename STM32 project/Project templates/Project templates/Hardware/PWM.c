...
//#define LED_CON(x) GPIO_WriteBit(LED_PROT,LED_PIN, (BitAction)(x));//构建函数来操作引脚，结构为：GPIO_WriteBit(IIC_PROT, IIC_SCL, (BitAction)(Bit));
//                                                                                                               // 对应区域//对应引脚//相关数值（(BitAction)用于限制数值范围）
...
void PWM_Init(void )
{
   ...
   //初始化引脚
    ...
    GPIO_Instructure.GPIO_Pin=GPIO_Pin_0;//引脚为PA0
  ...
  //定时器初始化结构体
   ...
   TIM_TimeBaseInitstructure.TIM_Period=100-1;//定时器周期（ARR值设定）
   TIM_TimeBaseInitstructure.TIM_Prescaler=720-1;//预分频系数值（PSC值）
   ...
   TIM_OCInitstructure.TIM_Pulse=0;//指定要加载到捕获比较寄存器中的脉冲值(开始输出值)
   //初始化定时器通道
...
{
    TIM_SetCompare1(TIM2,time);//操作TIM2的一号通道
}
